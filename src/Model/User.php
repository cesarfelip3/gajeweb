...
class User extends BaseModel
{
    ...
    public function getFollowerList($data)
    {
        ...
        $sql = "SELECT DISTINCT usr.*, fol.is_mutual AS is_mutual FROM {$this->table} usr INNER JOIN {$this->table_user_follow} fol ON usr.user_uuid=fol.user_followed_uuid WHERE fol.user_followed_uuid=? ORDER BY fol.create_date DESC LIMIT {$limit}";
    ...
    public function getFollowingList($data)
    {
        ...
        $sql = "SELECT DISTINCT usr.* FROM {$this->table} usr INNER JOIN {$this->table_user_follow} fol ON usr.user_uuid=fol.user_following_uuid WHERE fol.user_following_uuid=? ORDER BY fol.create_date DESC LIMIT {$limit}";
        $stmt = $this->db->prepare($sql);
        $stmt->bindValue (1, $data["user_uuid"]);
        $stmt->execute();
        $result = $stmt->fetchAll();
